...
This is very easy - you don't have to perform any source code analysis - just scroll to the bottom and along the copyright we see the JS framework (not the best security practice :( ).
...
Luckily there are only 2 sources, unlucky that one of them has 88000 entries...
...
Maybe the first IP is the answer, but if we look closely and try to filter for "*admin*" since the admin portal was mentioned in the task:
...
The answer is the src IP that made about 2100 requests and tried to enumerate the web server pages:
